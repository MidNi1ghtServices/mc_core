...
function openLifeinvader(data) {
    ...
    liConfig.allowName = !!data.allowName;

    document.getElementById("liServerName").innerText = (data.serverName || "Lifeinvader").toUpperCase();
    document.getElementById("liBadge").innerText = data.badge || "WERBUNG";
    ...
    // Öffentlich/Privat-Umschalter (ersetzt die alte "Mit Namen versehen"-Checkbox)
    liPublicMode = liConfig.allowName ? !!data.nameDefault : false;
    document.getElementById("liPrivateBtn").classList.toggle("hidden", !liConfig.allowName);
    updateLiModeButtons();

    // Spieler-Box oben rechts
    document.getElementById("liPlayerName").innerText = data.playerName || "-";
    document.getElementById("liPlayerCash").innerText = `${data.cash || 0}$`;
    const timerEl = document.getElementById("liPlayerTimer");
    const timerSep = document.getElementById("liPlayerTimerSep");
    if (data.cooldownRemaining && data.cooldownRemaining > 0) {
        timerEl.innerText = formatLiTimer(data.cooldownRemaining);
    ...
    document.getElementById("liAdsCount").innerText = data.adsPosted || 0;
...
function formatLiTimer(totalSeconds) {
...
document.getElementById("liPrivateBtn").onclick = function () {
    if (!liConfig.allowName) return;
    liPublicMode = false;
    updateLiModeButtons();
};
...
function showLifeinvaderResult(data) {
    ...
    if (data.success) {
        ...
        const adsCountEl = document.getElementById("liAdsCount");
        adsCountEl.innerText = (parseInt(adsCountEl.innerText, 10) || 0) + 1;

        if (typeof data.cash === "number") {
            document.getElementById("liPlayerCash").innerText = `${data.cash}$`;
        }
    }
